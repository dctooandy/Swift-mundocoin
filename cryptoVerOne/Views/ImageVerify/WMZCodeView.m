...
- (void)CodeTypeImageView{
    ...
    [self addSubview:({
        ...
        self.tipLabel.font = [UIFont fontWithName:@"PingFangTC-Regular" size:16];
        self.tipLabel;
    })];
    ...
    [self addSubview:({
        ...
        self.slider.layer.cornerRadius = 30/4;
        self.slider;
    })];
    
    [self addSubview:({
        self.refresh.frame = CGRectMake(self.width-margin-50, CGRectGetMaxY(self.slider.frame)+margin, 40, 40);
        [self.refresh setImage:[UIImage imageNamed:@"icon-refresh"] forState:UIControlStateNormal];
...
- (void)refreshAction{
    ...
    NSArray *images = [NSArray arrayWithObjects: @"puzzie-a", @"puzzie-b", @"puzzie-c", @"puzzie-d", nil];
    
    self.name = images[[self getRandomNumber:0 to:3]];
...
- (WMZSlider *)slider{
    if (!_slider) {
        ...
        _slider.thumbTintColor = [UIColor colorWithRed:0.56 green:0.45 blue:0.85 alpha:1.0];
        _slider.minimumTrackTintColor = [UIColor colorWithRed:0.56 green:0.45 blue:0.85 alpha:0.8];
    }
    return _slider;
}
...
- (CGRect)trackRectForBounds:(CGRect)bounds{
    ...
    CGFloat minY = height - height / 2;
    return CGRectMake(0, minY, self.frame.size.width, height);
}

- (CGRect)thumbRectForBounds:(CGRect)bounds trackRect:(CGRect)rect value:(float)value {
    
    return CGRectOffset([super thumbRectForBounds:bounds trackRect:rect value:value], 0, 0);
}
